...
const CONFIG = {
    // 請填入您的 GCP Client ID
    CLIENT_ID: '254391004976-giqndhj9hdf2h61pik5q2bsc05mtp7j7.apps.googleusercontent.com',
    // 請填入您的 Google Sheet ID
    SPREADSHEET_ID: '1TLd9a7jMwRR4-0TGgyNczbCNgUwIIDEXPgeXc8AdB6I',
...
/**
 * 封裝 Google Sheets API 呼叫 (透過原生的 fetch 取代 GAPI)
 */
async function fetchGoogleSheets(action, range, method = 'GET', body = null) {
    if (!currentAccessToken) throw new Error("No access token available.");
    
    let url = `https://sheets.googleapis.com/v4/spreadsheets/${CONFIG.SPREADSHEET_ID}/values/${range}`;
    if (action === 'append') {
...
async function checkUserPermission(email) {
    try {
        const response = await fetchGoogleSheets('get', `${SHEETS.USERS}!A:C`);
...
async function getTodayRestaurants() {
    const response = await fetchGoogleSheets('get', `${SHEETS.TODAY}!A:A`);
    const rows = response.values;
...
async function getAllMenu() {
    const response = await fetchGoogleSheets('get', `${SHEETS.MENU}!A:D`);
    const rows = response.values;
    // 轉換為物件陣列
...
async function submitOrder(restaurant, foodName, price, btnElement) {
    ...
    try {
        await fetchGoogleSheets('append', `${SHEETS.ORDERS}!A:F`, 'POST', {
            values: [orderData]
        });
...
async function saveTodayConfig() {
    ...
    try {
        ...
        await fetchGoogleSheets('clear', `${SHEETS.TODAY}!A2:A`, 'POST');

        if (selected.length > 0) {
            // 寫入新設定
            const values = selected.map(r => [r]);
            await fetchGoogleSheets('append', `${SHEETS.TODAY}!A2`, 'POST', {
                values: values
            });
        }
